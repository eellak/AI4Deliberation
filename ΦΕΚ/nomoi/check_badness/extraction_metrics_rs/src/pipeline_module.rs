...
pub fn run_complete_pipeline(
    ...
    let pipeline_result = (|| {
        ...
        if !output_report_csv_str.is_empty() {
            // (CSV writing logic - same as before, just ensure it's using final_report_entries from this stage)
            println!("Rust: Preparing to write final CSV report to: {}", output_report_csv_str);
            if let Some(parent_dir) = Path::new(output_report_csv_str).parent() {
                if !parent_dir.as_os_str().is_empty() && !parent_dir.exists() {
                   println!("Rust: Creating parent directory for CSV: {}", parent_dir.display());
                   fs::create_dir_all(parent_dir).map_err(|e| PyValueError::new_err(format!("Failed to create parent for CSV {}: {}", parent_dir.display(), e)))?;
                   println!("Rust: Parent directory for CSV created.");
                }
            }
            let output_report_csv_str_owned = output_report_csv_str.to_string(); // Ensure owned string for from_path
            let mut wtr = csv::WriterBuilder::new().has_headers(true).from_path(output_report_csv_str_owned)
                .map_err(|e| PyValueError::new_err(format!("Failed to create CSV writer for path '{}': {}", output_report_csv_str, e)))?;

            wtr.write_record(&["File Name", "Badness", "Greek Percentage", "Latin Percentage"])
                .map_err(|e| PyValueError::new_err(format!("CSV header write error: {}", e)))?;
            println!("Rust: CSV headers written successfully to {}.", output_report_csv_str);

            for entry in final_report_entries {
                wtr.serialize(entry).map_err(|e| PyValueError::new_err(format!("CSV record serialization error: {}",e)))?;
            }
            println!("Rust: All Stage 4 report entries serialized.");
            wtr.flush().map_err(|e| PyValueError::new_err(format!("CSV flush error: {}", e)))?;
            println!("Rust: CSV writer flushed successfully. Report should be complete at {}.", output_report_csv_str);
        } else {
            println!("Rust: CSV output path is empty, skipping CSV report generation for Stage 4.");
        }
